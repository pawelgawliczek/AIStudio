...
describe('MetricsUtils - Pure Utility Functions', () => {
  describe('calculateDateRange', () => {
    it('should calculate last 7 days for LAST_7_DAYS', () => {
      const result = calculateDateRange(DateRange.LAST_7_DAYS);
    ...
    it('should calculate last 30 days for LAST_30_DAYS', () => {
      const result = calculateDateRange(DateRange.LAST_30_DAYS);
      const daysDiff = Math.floor((result.endDate.getTime() - result.startDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    it('should calculate last 90 days for LAST_90_DAYS', () => {
      const result = calculateDateRange(DateRange.LAST_90_DAYS);
      const daysDiff = Math.floor((result.endDate.getTime() - result.startDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    it('should calculate last 6 months for LAST_6_MONTHS', () => {
      const result = calculateDateRange(DateRange.LAST_6_MONTHS);
      const monthsDiff =
    ...
    it('should handle CUSTOM range with provided dates', () => {
      ...
      const result = calculateDateRange(DateRange.CUSTOM, customStart, customEnd);
    ...
    it('should default to ALL_TIME for all time range', () => {
      const result = calculateDateRange(DateRange.ALL_TIME);
    ...
    it('should handle CUSTOM range without dates by returning current date', () => {
      const result = calculateDateRange(DateRange.CUSTOM);
  ...
  describe('getComplexityFilter', () => {
    it('should return [1, 2] for LOW complexity', () => {
      const result = getComplexityFilter(ComplexityBand.LOW);

      expect(result).toEqual([1, 2]);
    });

    it('should return [3] for MEDIUM complexity', () => {
      const result = getComplexityFilter(ComplexityBand.MEDIUM);

      expect(result).toEqual([3]);
    });

    it('should return [4, 5] for HIGH complexity', () => {
      const result = getComplexityFilter(ComplexityBand.HIGH);

      expect(result).toEqual([4, 5]);
    });

    it('should return null for ALL complexity (no filter)', () => {
      const result = getComplexityFilter(ComplexityBand.ALL);

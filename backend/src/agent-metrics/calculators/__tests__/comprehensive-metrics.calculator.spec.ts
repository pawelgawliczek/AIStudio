 ...
 * This calculator extracts logic from calculateComprehensiveMetrics (lines 1051-1168)
 */
...
describe('ComprehensiveMetricsCalculator', () => {
  describe('calculateComprehensiveMetrics', () => {
    it('should calculate token metrics from component runs', () => {
      ...
      const result = calculateComprehensiveMetrics(workflowRuns);
    ...
    it('should count unique files modified', () => {
      const workflowRuns = [
        createWorkflowRun({
          componentRuns: [
            createComponentRun({ filesModified: ['file1.ts', 'file2.ts'] }),
            createComponentRun({ filesModified: ['file2.ts', 'file3.ts'] }), // file2.ts duplicated
          ],
        }),
      ];
      ...
      // Should deduplicate: file1.ts, file2.ts, file3.ts = 3 files
      expect(result.codeImpact.filesModified).toBe(3);
    });
    ...
    it('should handle empty workflow runs array', () => {
      ...
      expect(result.costValue.storiesCompleted).toBe(0);
    });
    ...
    it('should calculate rework cost based on code churn', () => {
      ...
      // Rework cost = totalCost * (codeChurnPercent / 100)
      // Since we don't track churn in this calculator, it should be 0 or calculated separately
...
// Type definition for the calculator function (to be implemented)
interface ComprehensiveMetricsDto {
  tokens: {
    inputTokens: number;
    outputTokens: number;
    cacheRead: number;
...
declare function calculateComprehensiveMetrics(workflowRuns: any[]): ComprehensiveMetricsDto;

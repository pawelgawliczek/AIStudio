...
describe('MetricsAggregationService', () => {
  ...
  describe('aggregateByWorkflow', () => {
    it('should group workflow runs by workflow ID', () => {
      ...
      expect(result[0].workflowId).toBe(workflow1Id);
      expect(result[0].totalRuns).toBe(2);
      expect(result[1].workflowId).toBe(workflow2Id);
  ...
  describe('aggregateByStory', () => {
    it('should group workflow runs by story ID', () => {
      ...
      expect(result).toHaveLength(2);
      expect(result[0].storyId).toBe(story1Id);
      expect(result[0].storyKey).toBe('ST-1');
    });
  ...
  describe('aggregateByEpic', () => {
    it('should group workflow runs by epic ID', () => {
      ...
      expect(result).toHaveLength(2);
      expect(result[0].epicId).toBe(epic1Id);
      expect(result[1].epicId).toBe(epic2Id);
    });
...
function createWorkflowRun(partial: Partial<any> = {}): any {
  return {
    ...
    workflow: partial.workflow || { name: 'Default Workflow' },
    story: partial.story || { key: 'ST-999', title: 'Default Story' },
...
function createComponentRun(partial: Partial<any> = {}): any {
  return {
    ...
    component: partial.component || { name: 'Default Component' },
...
// Type definitions for service interface
interface WorkflowMetricsSummaryDto {
  workflowId: string;
  workflowName: string;
  totalRuns: number;
  metrics: any;
...
interface StoryMetricsSummaryDto {
  storyId: string;
  storyKey: string;
  storyTitle: string;
...
interface EpicMetricsSummaryDto {
  epicId: string;
  epicKey: string;
  epicTitle: string;
  totalStories: number;
...
interface AgentMetricsSummaryDto {
  agentName: string;
  componentId: string;
...
// Mock service class definition (to be implemented)
declare class MetricsAggregationService {

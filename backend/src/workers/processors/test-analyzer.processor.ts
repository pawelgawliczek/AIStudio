...
export class TestAnalyzerProcessor {
  ...
  async calculateCoverageGaps(job: Job<{ projectId: string }>) {
    ...
    try {
      ...
      const projectCoverage: ComponentCoverageData = {
        layer: 'project',
        component: projectId,
        totalFiles: 0,
        totalLOC: 0,
      ...
      // 4. Count test cases by type
      for (const testCase of testCases) {
        if (testCase.testLevel === 'unit') projectCoverage.testCases.unit++;
        else if (testCase.testLevel === 'integration') projectCoverage.testCases.integration++;
        else if (testCase.testLevel === 'e2e') projectCoverage.testCases.e2e++;
      }

      // 5. Generate coverage gap report
      const gaps = [{
        ...projectCoverage,
        coveragePercentage: projectCoverage.totalLOC > 0
          ? (projectCoverage.coveredLOC / projectCoverage.totalLOC) * 100
          : 0,
      }];

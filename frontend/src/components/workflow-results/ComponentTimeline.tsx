      ...
      <div className="space-y-3">
        {componentRuns.map((run, index) => (
          <div key={run.id} className="border border-border rounded-lg overflow-hidden">
            {/* Component Header */}
            <div
              className="bg-bg-secondary p-4 cursor-pointer hover:bg-gray-100 transition"
                ...
                <div className="flex items-start space-x-3 flex-1">
                  ...
                  <div className="flex-1">
                    ...
                    {run.componentDescription && (
                      <p className="text-sm text-muted mt-1">{run.componentDescription}</p>
                    )}
            ...
            {expandedRun === run.id && (
              <div className="p-4 bg-card border-t border-border">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                  ...
                  <div>
                    <div className="text-xs text-muted">Iterations</div>
                    <div className="text-sm font-medium">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                  <div className="bg-blue-50 rounded p-2">
                    <div className="text-xs text-accent">Tokens/LOC</div>
                ...
                {run.filesModified && run.filesModified.length > 0 && (
                  <div className="mb-4">
                    ...
                    <div className="bg-bg-secondary rounded p-2 max-h-32 overflow-y-auto">
                      {run.filesModified.map((file, i) => (
                        <div key={i} className="text-xs font-mono text-fg py-1">
                          {file}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-1">
                      {run.commits.map((commit, i) => (
                        <div key={i} className="text-xs font-mono bg-bg-secondary rounded px-2 py-1">
                          {commit}
                        </div>
                      ))}
                    </div>
                ...
                {/* Error Message */}
                {run.errorMessage && (
                  <div className="p-3 bg-red-100/50 border border-red-200 rounded">
                ...
                {run.output && (
                  <div className="mt-4">
                    <div className="text-sm font-medium text-fg mb-2">Output</div>

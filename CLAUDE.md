...
3. User provides test URL: `http://127.0.0.1:5174` or `http://127.0.0.1:3001` or `https://test.vibestudio.pawelgawliczek.cloud`
4. File `TEST_MODE_ACTIVE.md` exists in current worktree

**IMPORTANT: Test Environment URLs (Same Environment)**
- `http://127.0.0.1:5174` = `https://test.vibestudio.pawelgawliczek.cloud` (frontend)
...
3. User provides production URL: `https://vibestudio.pawelgawliczek.cloud` (NO "test." prefix!)
...
```typescript
// .env or .env.docker
VITE_API_URL="https://vibestudio.pawelgawliczek.cloud"  // NO /api suffix
VITE_WS_URL="wss://vibestudio.pawelgawliczek.cloud"      // NO /api suffix

// frontend/src/services/*.service.ts
const response = await api.get(`/api/projects/${projectId}/components`);  // Include /api
```

**Result:** `https://vibestudio.pawelgawliczek.cloud/api/projects/{id}/components` ✅
